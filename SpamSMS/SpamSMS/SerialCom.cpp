...
const char* CSerialCom::WriteByte(const char* szMessage)
{
	iBytesWritten=0;
	...
	if(WriteFile(hComm, (LPCVOID)&szSend, strlen(szSend), &iBytesWritten, NULL)==0)
		return NULL;
	else 
	{
		memset(&szSend, 0, 140);
		if(ReadFile(hComm, (LPVOID)&szReturn, 140, &dwBytesTransferred, 0))
		{
